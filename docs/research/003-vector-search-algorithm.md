...
| :-------------- | :--------- | :--------- | :---------- | :------------------------------------------------------------------------------ |
| **Milvus**      | ~15,000    | 8ms        | ~2 GB       | **F1 赛车**。为性能而生，综合实力强悍，但维护较复杂（组件多）。                 |
| **Weaviate**    | ~8,000     | 15ms       | ~2.2 GB     | **豪华 SUV**。功能丰富（多模态/模块化），舒适但车身略重。                       |
| **Qdrant**      | ~12,000    | 10ms       | ~1.8 GB     | **高性能跑车**。Rust 引擎极稳，单机性能优异，启动快，轻量灵活。                 |
| **pgvector**    | ~3,000     | 25ms       | ~1.5 GB     | **家用轿车**。从买菜（CRUD）到代步（搜索）一台车搞定，生态极好，够用就好。      |
| **VectorChord** | ~15,000    | 8ms        | ~1.2 GB     | **改装神车 (GTI)**。针对 PG 极致优化的“钢炮”，在特定赛道（1M 规模）甚至能超跑。 |
...
| :----------- | :----- | :----- | :----- | :--------------------------------------------------------------------------------- |
| **Qdrant**   | 1 亿+  | ~4,000 | < 10ms | **越野拉力车**。Rust 引擎在 SSD 上依然表现出色，Memmap 技术让它能跑烂路。          |
| **Milvus**   | 1 亿+  | ~2,000 | < 5ms  | **高铁网络**。重型基建（分布式微服务），虽然造价高、维护难，但兼具极速与超大运力。 |
| **Weaviate** | 1 亿+  | ~1,500 | < 20ms | **模块化列车**。虽然极速不是最快，但稳重可靠，适合混合负载。                       |
| **DiskANN**  | 10 亿  | ~500   | < 10ms | **重型卡车**。单机拉 10 亿数据的性价比之王，速度慢点但能通过。                     |
