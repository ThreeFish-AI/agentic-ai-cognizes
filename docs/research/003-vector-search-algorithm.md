...
> [!TIP] **图示说明**
>
> - **"猫" Cat (0.8, 0.7)** 与 **"狗" Dog (0.85, 0.75)** 位于右上象限，语义距离仅 **0.07**
> - **"汽车" Car (0.1, 0.1)** 位于左下象限，与"猫"语义距离达 **0.92**
> - 这直观体现了语义相似对象在向量空间中聚集的特性

这种特性使得我们可以：

1. **语义搜索**：通过向量距离找到语义相关的内容 —— 如搜索"如何提升代码质量"能匹配到"代码重构最佳实践"
2. **知识检索**：从大规模知识库中检索相关信息 —— 如 RAG 系统从百万文档中秒级定位答案来源
3. **去重聚类**：识别和分组相似内容 —— 如新闻聚合平台自动归类同一事件的不同报道
