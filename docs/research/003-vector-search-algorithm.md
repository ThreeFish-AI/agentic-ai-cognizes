...
IVFFlat（Inverted File with Flat vectors）是 IVF 家族中最基础的成员，它就像是一个 **珍藏原始手稿的图书馆**。

- **IVF (倒排)**：书本按主题分架摆放（聚类），找书时只需看特定的几个书架。
- **Flat (平铺)**：书架上放的是厚重的 **原始手稿**（完整向量），没有做任何删减或压缩。

这种方式虽然保证了内容的 **原汁原味**（精度高），但也意味着书架必须很大（内存占用高），且每次对比都要阅读全文（计算慢）。

> [!NOTE] 💡 辨析：IVF 与 IVFFlat
>
> 很多人容易混淆这两个概念，其实它们是 **“骨架”** 与 **“血肉”** 的关系：
>
> - **IVF (索引结构)**：决定了**“怎么找”**（通过聚类缩小范围）。
> - **Flat (存储方式)**：决定了**“存什么”**（存储未压缩的原始向量）。
>
> **IVFFlat** 就是两者的结合。如果我们把存储方式换成由 PQ（乘积量化）压缩后的编码，就变成了 **IVF-PQ**（下一节介绍），那时候书架上放的就是“缩印本”了。
