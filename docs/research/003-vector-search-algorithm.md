...
| 数据库 (车队)   | QPS (时速)   | P99 (操控) | 内存 (油耗) | 点评                                           |
| :-------------- | :----------- | :--------- | :---------- | :--------------------------------------------- |
| **Milvus**      | ~15,000      | 8ms        | ~2 GB       | **F1 赛车**，综合性能强悍                      |
| **Qdrant**      | ~12,000      | 10ms       | ~1.8 GB     | **高性能跑车**，Rust 引擎极稳                  |
| **Weaviate**    | ~8,000       | 15ms       | ~2.2 GB     |
| **Pinecone**    | N/A (云服务) | 7ms        | N/A         |
| **pgvector**    | ~3,000       | 25ms       | ~1.5 GB     | **家用轿车**，方便无论是买菜还是代步（生态好） |
| **VectorChord** | ~15,000      | 8ms        | ~1.2 GB     | **改装神车**，针对 PG 极致优化                 |

#### 7.2.2 亿级赛道 (SSD Base)

| 数据库       | 规模  | QPS    | P99 延迟 | 备注                                             |
| :----------- | :---- | :----- | :------- | ------------------------------------------------ |
| **DiskANN**  | 10 亿 | 500+   | < 10ms   | **重型卡车**，拉得多（跑 SSD），虽然慢点但能装。 |
| **Milvus**   | 1 亿  | 2,000+ | < 5ms    | **车队集群**，靠堆数量（分布式）解决负载问题。   |
| **Pinecone** | 10 亿 | N/A    | < 10ms   | Pod 类型影响                                     |

### 7.3 索引算法：选什么车？

| 索引类型    | 类比车型      | 查询速度           | 成本 (内存)      | 召回率 | 适用场景                        |
| :---------- | :------------ | :----------------- | :--------------- | :----- | :------------------------------ |
| **Flat**    | **徒步**      | 极慢 (O(N))        | 高 (100%)        | 100%   | < 10 万，数据极少，决不能出错   |
| **IVFFlat** | **公交车**    | 快 (O(√N))         | 高 (100%)        | 99%    | < 1000 万，数据量中等，内存够用 |
| **IVFPQ**   |               | 快 (O(√N))         | **极低** (5%)    | 90-98% | 无限制                          |
| **HNSW**    | **高铁/飞机** | **极快** (O(logN)) | **极高** (150%+) | 99%+   | < 1 亿，土豪首选，追求极致速度  |
| **DiskANN** | **货轮**      | 较慢 (O(logN))     | **极低** (10%)   | 95%+   | 10 亿+，海量数据，内存放不下    |
