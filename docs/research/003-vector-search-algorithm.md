...
为了拯救这位“天才但与世隔绝且脑容量有限的博士”，我们需要将互联网这个“图书馆”中“书本”的知识**外挂**给他。但首先面临的问题是：计算机无法直接理解“书本”里的文字含义。

**Embedding（嵌入向量）** 便是解决这一难题的“数字翻译官”。它能将文本、图像、音频等非结构化数据，转译为计算机可计算的**语义坐标（高维向量）**，让机器“读懂”字面背后的含义<sup>[[4]](#ref4)</sup>。

> [!NOTE]
>
> **Embedding 的数学模型** 是一个映射函数 $f: X \rightarrow \mathbb{R}^d$，即将输入空间 $X$ 中的对象映射到 $d$ 维实数向量空间 $\mathbb{R}^d$，向量化的过程可以表示为：
>
> $$
...
> [!IMPORTANT]
>
> K-Means 在向量索引中的核心价值：构建 **"向量字典"（码本）**，实现数据的高倍率压缩。
>
> **核心概念：码本 (Codebook) 与 码字 (Codeword)**
>
> 想象一张包含 1600 万种颜色的照片（原始数据）。为了压缩，我们制作了一张只包含 256 种典型颜色的 **调色板（码本 Codebook）** 来近似表示原始的 1600 万种颜色。调色板中的每一种标准色块就是一个 **码字 (Codeword)**。
>
> - **压缩原理**：在存储时，不再记录每个像素复杂的 RGB 值（浮点数），而是仅记录它最接近的那个标准色在调色板中的 **编号 ID**。
> - **效果**：用极小的 ID（整数）替代了庞大的浮点数数组，虽然损失了微小的精度，但换取了数十倍的存储空间。
...
### 3.2 LSH（Locality Sensitive Hashing, 局部敏感哈希）
